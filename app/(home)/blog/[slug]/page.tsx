...
export default async function Page({ params }: { params: { slug: string } }) {
  ...
  style={{
    marginTop: "3rem", // Agrega un margen superior de 3rem
      ...
      <article className="container flex flex-col px-0 py-8 lg:flex-row lg:px-4">
        ...
        <div className="flex flex-col gap-4 border-l p-4 text-sm lg:w-[250px]">
          ...
          <div>
            ...
            <p className="font-medium">{new Date(page.data.date ?? page.file.name).toDateString()}</p>
          </div>
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const page = blog.getPage([resolvedParams.slug])
